...
- 当前版本：[v1.1.2](https://github.com/RSXLX/focus-pet/releases/tag/v1.1.2)
...
最新版本：[v1.1.2](https://github.com/RSXLX/focus-pet/releases/tag/v1.1.2)

| 平台 | 下载 | 说明 |
| --- | --- | --- |
| macOS Apple Silicon | [Release assets](https://github.com/RSXLX/focus-pet/releases/tag/v1.1.2) | 下载 DMG，包含完整桌宠应用。 |
| macOS Apple Silicon | [Release assets](https://github.com/RSXLX/focus-pet/releases/tag/v1.1.2) | 也提供 ZIP 归档。 |
| 校验信息 | [Release assets](https://github.com/RSXLX/focus-pet/releases/tag/v1.1.2) | 包含 SHA-256 manifest。 |
...
`diagnostics-bundle-output` 会检查 `summaryBoundaryIssues` 和 `summarySchemaValid`；`cloud-health` 会检查 Focus Pet Cloud `/healthz`，包括后端屏幕检查、TURN JSON 有效性和 TURN 状态；`npm run cloud:smoke` 是人工生产 smoke test，会创建两个临时 Cloud 用户、互加好友、打开 WSS、发送一次通话邀请，并调用 `/api/screen-check`，因为会写入生产 Cloud 数据，所以不放入自动 preflight；`error-log` 会报告 `openUnresolvedEntries`。诊断包只包含最新 20 个相关错误记录，用于排查问题，同时避免暴露高敏感内容。
...
| 发布说明 | [docs/releases/v1.1.2.md](docs/releases/v1.1.2.md) |
