...
- 当前版本：[v1.1.0](https://github.com/RSXLX/focus-pet/releases/tag/v1.1.0)
...
最新版本：[v1.1.0](https://github.com/RSXLX/focus-pet/releases/tag/v1.1.0)

| 平台 | 下载 | 说明 |
| --- | --- | --- |
| macOS Apple Silicon | [Release assets](https://github.com/RSXLX/focus-pet/releases/tag/v1.1.0) | 下载 DMG，包含完整桌宠应用。 |
| macOS Apple Silicon | [Release assets](https://github.com/RSXLX/focus-pet/releases/tag/v1.1.0) | 也提供 ZIP 归档。 |
| 校验信息 | [Release assets](https://github.com/RSXLX/focus-pet/releases/tag/v1.1.0) | 包含 SHA-256 manifest。 |
...
- Focus Pet Cloud 账号和通话：完整桌宠里可以创建稳定用户 ID、显示好友码、添加好友码，并通过认证 WebSocket 信令建立一对一 WebRTC 语音/视频通话。
- 更新通知：检查 GitHub Release 更新源，有新 DMG/ZIP 时推送系统通知；安装仍由用户手动确认。
...
Focus Pet 将本地陪伴聊天和 Cloud 实时通话分开处理：

- 微信式小聊天窗口：支持文字、媒体消息、宠物 GIF 分享，以及通过 `MediaRecorder` 录制的语音消息。桌面端支持按住说话，也支持语音快捷键 `Alt+R`。
- Cloud 实时通话：实时语音聊天和实时视频聊天使用 WebRTC；会话建立依赖认证 WebSocket 信令，生产环境必须配置 TURN 才能提升复杂网络下的连通性。

公开多人分发时，建议使用 Focus Pet Cloud，而不是让每台桌面端直接暴露服务。Cloud 后端提供稳定用户 ID、好友码配对、设备绑定 token、认证 WebSocket 信令，以及一对一 WebRTC 语音/视频所需的 ICE/TURN 配置。见 [Focus Pet Cloud](docs/focus-pet-cloud.md)。
...
`npm run verify:mac` 会验证 codesign、Gatekeeper 和 stapled notarization ticket；任一公开发布门禁失败都会以非零状态退出。
...
| 发布说明 | [docs/releases/v1.1.0.md](docs/releases/v1.1.0.md) |
