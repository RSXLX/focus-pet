...
系统默认不读取网页正文、不记录键盘输入、不截屏。只有用户手动开启屏幕监控并配置 LLM 后，才会按设定间隔截取屏幕缩略图并发送到用户配置的 OpenAI-compatible 视觉模型。
...
- 诊断摘要、屏幕监控和 LLM 连通性自检不在主进程启动时加载；只有用户打开诊断、手动测试屏幕监控、开启自动屏幕监控或点击 LLM 自检时，才加载对应模块。
...
屏幕监控 LLM 成功返回后的用户可见消息也沿用同一口径：`distracted` 表达为“可能偏离当前任务”，低置信度结果只展示状态，不主动触发强提醒。
...
本地日志默认保留 30 天，可在高级设置中调整为 1-365 天。系统每次写入新的活动样本、屏幕监控样本或运行日志条目后会按保留周期裁剪旧记录；时间字段缺失或无法解析的历史 JSONL 行会保留，旧的 `[time] message` 运行日志行会按兼容格式解析并参与同一保留窗口。

### 3.6 可选屏幕监控

屏幕监控默认关闭。开启后，系统会按设置间隔截取屏幕缩略图，并发送到用户配置的视觉 LLM。
...
AI 设置中可以切换“允许云端”或“仅本地”。在“仅本地”模式下，非 `localhost`、`127.0.0.1` 或本机回环地址的 LLM endpoint 会被视为未满足配置。Ollama 和本机 OpenAI-compatible provider 不要求 API key；请求头不会发送空的 `Authorization`。
...
- 监控日志只保存状态、摘要、结构化字段和截图策略摘要，不保存截图 data URL。

### 3.7 新手引导

桌面端提供“引导”入口，帮助用户在三种模式里选择初始使用方式：

- 基础模式：任务 + 宠物 + 前台 App 判断。用户可以一键完成基础模式，系统会关闭屏幕监控、复盘 LLM 和自动更新，保持本地低打扰使用。
...
- AI：云端请求模式、屏幕监控开关和间隔、屏幕模型提供方、LLM Endpoint 和 Model、复盘 LLM 提供方和配置、LLM 连通性自检、手动测试监控、屏幕权限入口。
- 社交：媒体大小上限、语音消息快捷键；邀请码、外部会话和通话配置由聊天面板管理。
- 高级：更新源 URL、自动检查更新、本地日志保留天数、数据目录、诊断摘要、权限引导。

首次打开设置时默认显示基础分组。AI、社交和高级分组集中承载屏幕监控、外部会话、通话、诊断和存储相关能力，避免低频高风险配置混在基础设置里。
