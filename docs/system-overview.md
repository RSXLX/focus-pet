...
- 自动检查由主进程定时执行，发现新版本时使用宠物气泡和系统通知提醒用户。
- 通知点击、宠物气泡和手动“检查更新”都会直接下载并打开最新 DMG/ZIP；只有更新源缺少可下载资产时才兜底打开 Release 页面。
- 应用不会静默替换自身或重启安装包，安装动作仍由用户确认。
...
公开下载默认使用 Focus Pet Cloud 账号入口：用户创建稳定 ID 后会得到 6 位数字好友码，可一键复制给对方。Cloud 模式当前支持文字消息、5MB 以内图片消息和 WebRTC 语音/视频通话；文件、语音消息和宠物动图仍属于本地聊天服务能力。
...
4. 本地聊天双方可以发送文本、图片、文件、语音消息和宠物动图；Cloud 好友当前支持文字和图片消息。宠物动图包括摸摸、喂食、专注、跳舞、休息、完成庆祝、哈哈开心、伸展、补水、冥想、复盘阅读、加油好棒、早安挥手、抱抱安慰、惊讶、哭哭、生气、忙碌、OK、想你和通话陪伴，素材来自短发毛衣、浅色洞洞鞋小人形象的独立图片包。
...
`release:preflight` 的 package 模式在 macOS 上执行 `npm run release:mac`，用于当前公开下载路径生成 ad-hoc signed DMG/ZIP/manifest。清单仍保留 `npm run package:mac`、`npm run sign:mac && npm run verify:mac` 和 `npm run notarize:mac && npm run verify:mac` 作为人工步骤；其中 Developer ID 签名和公证需要 Apple ID、Team ID 和 App 专用密码，只用于 Apple-notarized build。可选远端聊天/通话客户端 release 需要先部署 HTTPS `/client` 并设置 `REMOTE_CLIENT_URL`，再单独执行 `npm run release:mac:remote-client`，其中 URL 必须指向 `/client` 或 `/client/...` 路径；打包出的客户端只向与该 URL 精确同源的页面授予麦克风/摄像头权限，内嵌导航只保留同源 `/client` 页面，跳出范围的 http/https 导航交给系统浏览器，非 http/https 外链会被拒绝打开；该步骤不随 `--run package` 自动运行。`npm run package:mac:remote-client` 可单独生成 `.app`。Windows package 模式包含 `npm run package:win`，需要在 Windows 环境执行。
